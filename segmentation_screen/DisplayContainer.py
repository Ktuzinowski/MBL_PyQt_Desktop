...
class DisplayContainer(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        EventHandler().add_event_listener(Events.IMAGE_SELECTED, self.image_selected)
        EventHandler().add_event_listener(Events.GRADXY_SELECTED, self.gradxy_selected)
        EventHandler().add_event_listener(Events.CELLPROB_SELECTED, self.cellprob_selected)
        EventHandler().add_event_listener(Events.GRADZ_SELECTED, self.gradz_selected)
        EventHandler().add_event_listener(Events.MASKS_ON_TOGGLED, self.handle_masks_toggled)
        EventHandler().add_event_listener(Events.OUTLINES_ON_TOGGLED, self.handle_outlines_toggled)
        EventHandler().add_event_listener(Events.CALIBRATE_PRESSED, self.handle_calibration_button)
        EventHandler().add_event_listener(Events.RUN_MODEL_PRESSED, self.handle_run_model_pressed)
        EventHandler().add_event_listener(Events.AUTO_ADJUST_TOGGLED, self.handle_auto_adjust_toggled)
        EventHandler().add_event_listener(Events.SATURATION_SLIDER_ADJUSTED, self.handle_saturation_slider_adjusted)
    ...
    def handle_masks_toggled(self):
        print(Events.MASKS_ON_TOGGLED)

    def handle_outlines_toggled(self):
        print(Events.OUTLINES_ON_TOGGLED)

    def handle_calibration_button(self):
        print(Events.CALIBRATE_PRESSED)

    def handle_run_model_pressed(self):
        print(Events.RUN_MODEL_PRESSED)

    def handle_auto_adjust_toggled(self):
        print(Events.AUTO_ADJUST_TOGGLED)

    def handle_saturation_slider_adjusted(self):
        print(Events.SATURATION_SLIDER_ADJUSTED)
