    ...
    if 'flows' in dat:
        ...
        try:
            if parent.flows[0].shape[-3] != dat['masks'].shape[-2]:
                ...
                parent.flows[0] = cv2.resize(parent.flows[0].squeeze(), (Lx, Ly), interpolation=cv2.INTER_NEAREST)[
                    np.newaxis, ...]
                parent.flows[1] = cv2.resize(parent.flows[1].squeeze(), (Lx, Ly), interpolation=cv2.INTER_NEAREST)[
                    np.newaxis, ...]
            if parent.NZ == 1:
